...
const std::string App::Cols::_compatible = "compatible";
...
{"compatible","bool","boolean",1,0,0,1}
};
...
App::App(const Row &r, const ssize_t indexOffset) noexcept
{
    if(indexOffset < 0)
    {
        ...
        if(!r["compatible"].isNull())
        {
            compatible_=std::make_shared<bool>(r["compatible"].as<bool>());
        }
    }
    else
    {
        ...
        if(!r[index].isNull())
        {
            compatible_=std::make_shared<bool>(r[index].as<bool>());
        }
    }
...
App::App(const Json::Value &pJson, const std::vector<std::string> &pMasqueradingVector) noexcept(false)
{
    ...
    if(!pMasqueradingVector[3].empty() && pJson.isMember(pMasqueradingVector[3]))
    {
        ...
        {
            compatible_=std::make_shared<bool>(pJson[pMasqueradingVector[3]].asBool());
        }
    }
}
...
App::App(const Json::Value &pJson) noexcept(false)
{
    ...
    if(pJson.isMember("compatible"))
    {
        dirtyFlag_[3]=true;
        if(!pJson["compatible"].isNull())
        {
            compatible_=std::make_shared<bool>(pJson["compatible"].asBool());
        }
    }
}
...
const bool &App::getValueOfCompatible() const noexcept
{
...
const std::shared_ptr<bool> &App::getCompatible() const noexcept
{
    return compatible_;
}
void App::setCompatible(const bool &pCompatible) noexcept
{
    compatible_ = std::make_shared<bool>(pCompatible);
    dirtyFlag_[3] = true;
}
...
Json::Value App::toJson() const
{
    ...
    if(getCompatible())
    {
        ret["compatible"]=getValueOfCompatible();
    }
    else
    {
        ret["compatible"]=Json::Value();
    }
    return ret;
}
    ...
    const std::vector<std::string> &pMasqueradingVector) const
{
    ...
    if(pMasqueradingVector.size() == 4)
    {
        ...
        if(!pMasqueradingVector[3].empty())
        {
            if(getCompatible())
            {
                ret[pMasqueradingVector[3]]=getValueOfCompatible();
            }
            else
            {
...
bool App::validateJsonForCreation(const Json::Value &pJson, std::string &err)
{
    ...
    if(pJson.isMember("compatible"))
    {
        if(!validJsonOfField(3, "compatible", pJson["compatible"], err, true))
            return false;
    }
    return true;
}
...
bool App::validateJsonForUpdate(const Json::Value &pJson, std::string &err)
{
    ...
    if(pJson.isMember("compatible"))
    {
        if(!validJsonOfField(3, "compatible", pJson["compatible"], err, false))
            return false;
    }
    return true;
}
