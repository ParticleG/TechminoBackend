...
namespace tech::structures {
    ...
    public:
        ...
        bool isFull(const std::string &roomID);

        bool isEmpty(const std::string &roomID);
    ...
    private:
        std::unordered_map<std::string, std::shared_ptr<BaseRoom>> _roomIDMap;
        mutable SharedMutex _sharedMutex;

        drogon::SubscriberID _subscribeToRoom(const std::string &roomID, typename BaseRoom::MessageHandler &&handler, const std::shared_ptr<tech::structures::Player> &player);
    };
}
